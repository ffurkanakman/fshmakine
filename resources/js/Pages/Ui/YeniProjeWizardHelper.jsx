...
export const projectFormSchema = [
  ...
  Yup.object({
    ...
    companyName: Yup.string().when(['customerType', 'isNewCustomer'], {
      is: (customerType, isNewCustomer) => customerType === 'new' || isNewCustomer === true,
      then: Yup.string().required('Firma Adı zorunludur'),
      otherwise: Yup.string(),
    }),
    authorizedPerson: Yup.string().when(['customerType', 'isNewCustomer'], {
      is: (customerType, isNewCustomer) => customerType === 'new' || isNewCustomer === true,
      then: Yup.string().required('Yetkili Adı Soyadı zorunludur'),
      otherwise: Yup.string(),
    }),
    phoneNumber: Yup.string().when(['customerType', 'isNewCustomer'], {
      is: (customerType, isNewCustomer) => customerType === 'new' || isNewCustomer === true,
      then: Yup.string().required('Telefon Numarası zorunludur'),
      otherwise: Yup.string(),
    }),
    email: Yup.string().when(['customerType', 'isNewCustomer'], {
      is: (customerType, isNewCustomer) => customerType === 'new' || isNewCustomer === true,
